...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ...
   if (growth_calc) then
      ...
      mass_trunk(new) = (0.051d0 * wood_rho(pft(new)) * tree_h(new) &
                        ...
                        * 1000.0d0
      mass_root(new) = 100.0
      mass_coarse_root(new) = mass_root(new) * ((1.0d0 - fine_root_ratio(pft(new))) / fine_root_ratio(pft(new)))
      if (pr_s_a(pft(new)) < 0.0d0) then ! For Rhizophora genus
     ...
     crown_area(new) = PI * (crown_diameter(new)/2)**2
     mass_leaf(new) = 3.0 * crown_area(new) / SLA(pft(new))  ! <= Value specified for the calculation is LAI.
     mass_trunk(new) = (0.051d0 * wood_rho(pft(new)) * tree_h(new) &
                       *(dbh_sapwood(new)*100.0d0 + dbh_heartwood(new)*100.0d0)**2) &
                       * 1000.0d0
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ...
   if (mass_leaf(no) < 1.1d0 .or. mass_root(no) < 1.1d0 ) then
      write(*,*) 'DIE!!'
      write(*,*) 'species', pft(no), 'DBH', dbh_heartwood(no)+dbh_sapwood(no), &
                 'trunk', mass_trunk(no), 'stock', mass_stock(no), &
                 'leaf', mass_leaf(no), 'root', mass_root(no), 'coarse root', mass_coarse_root(no)
      death_list(no) = .true.
   end if

   ! Kill grown tree with very small crown area

   if (age(no) > 5 .and. crown_area(no) < 0.05d0) then
      death_list(no) = .true.
   end if
