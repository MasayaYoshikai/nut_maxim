...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ...
   ! Calculation using JMA observation data
...
!write(*,*) 'radnet_long', radnet_long
!write(*,*) 'irsoi in SEIB', irsoi
!write(*,*) 'irveg in SEIB', irveg
!write(*,*) 'irleaf in SEIB', irleaf
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   real :: x_nir, y_nir
   real :: rad_diffuse_nir  ! Diffused NIR radiation at mid-day at canopy top (W/m2)
   real :: rad_direct_nir   ! Direct NIR radiation at mid-day at canopy top (W/m2)
   real :: rad_nir          ! Total NIR radiation at mid-day at canopy top (W/m2)
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MY:add
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ir_tree_nir = (rad_direct_nir/rad_nir)*exp(-1.0*x_nir) + (rad_diffuse_nir/rad_nir)*exp(-1.0*y_nir)
