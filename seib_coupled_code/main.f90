...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ...
   ! Flag of tree presence (0: No trees, 1: There are trees)
   ! When there are no trees, soil water variables are set to be constant.
...
!write(*,*) 'n_spe', n_spe
!write(*,*) 'rhol_vis, rhol_nir', rhol_vis, rhol_nir
!write(*,*) 'taul_vis, taul_nir', taul_vis, taul_nir
!write(*,*) 'emleaf, xl', emleaf, xl
!write(*,*) 'kbm_vis, kbm_nir', kbm_vis, kbm_nir

!write(*,*) 'td', td
!write(*,*) 'dpai_layer', dpai_layer(2,:)
!write(*,*) 'albvegb_vis, albvegb_nir', albvegb_vis, albvegb_nir

!   !---------------------------------------------------------------------
!   ! Longwave radiation transfer through canopy <= Removed.
!   !---------------------------------------------------------------------
!
!   ! メモ
!   ! 1. Bonan et al. (2014) のモデルの longwave radiation transfer の計算コードをほとんどその
!   !    まま使用している。
!   ! 2. transmittance of difffuse radiationには、鉛直方向からのradiationしか考慮していない。
!   !    これは、SEIB-DGVMのdiffused radiationと同じ考え方。
!   ! 3. 一次元的な計算なので、水平方向に均一なキャノピーを前提としている。
!   !    しかし、SEIB-DGVM の diffused radiation の計算でも同じような前提の上で計算しているので、よしとする。
!   ! 4. 1日に一回計算する。これは、短波放射に比べ、長波放射の日周変動は小さいと考えられるため。
!   !    SEIB-DGVMでもlongwave radiationの計算は1日一回なので、よしとする。
!   ! 5. Plot-scale leaf temperature (K) (tleaf_plot) という変数を定義している。
!   !    葉からの長波放射の算出に必要となる。葉の温度は日周変動しているはずだが、それを考慮すると複雑なので、
!   !    考慮しない。また、簡単のため、ここでは葉の温度は気温に等しいとする。
!   !    一応、SPAC-Photosynthesis model で各木の葉っぱの温度の鉛直分布を計算しているので、
!   !    葉っぱの温度の計算値を使うこともできると思う。その場合、各木の葉っぱの温度をその木がそのレイヤーの
!   !    LAIに占める葉っぱ面積で重みづけして、plot-scale leaf temperature の鉛直分布を求めれば良いと思う。
!   !    でも、そこまでするほどのことでも無いと思う。
!   ! 6. 地面の表面温度には一定値を与えている。ほんとは、熱収支や、潮汐の影響とかを考慮しなきゃいけないはず。
!
!!   tg = 298.15d0
!   tg = time_series_tair((doy-1)*24 + 1)
!   tleaf_plot = time_series_tair((doy-1)*24 + 1)
!   irsky = time_series_long_rad((doy-1)*24 + 1)
!
!   ! When there is canopy layer
!   if (nbot > 0) then
!      call longwave_rad ( &
!                                           ! *** Input ***
!      n_spe                           , &  ! Number of mangrove species
...
! write(*,*) 'nbot, ntop, lai, rwind_profile', nbot, ntop, sum(dpai_layer_sum), rwind_profile
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MY:add
   ...
   ! SPAC-photosynthesis => woody respiration => purge crown bottom layer
   ! => leaf and fine root turnover => stock biomass reload
   ! => biomass investment based on tree growth optimization
   ! => change of tree morphology (diameter and height)
   ! => New crown diameter and area based on allometric relation and
   !    spatial limitation
   ...
   dble(ir_tree)                   , &  ! VIS-interruption-coefficient by tree corwn (0.0 - 1.0). lower => more absorptance.
   dble(ir_tree_nir)               , &  ! NIR-interruption-coefficient by tree corwn (0.0 - 1.0). lower => more absorptance.
   irsoi                           , &  ! Absorbed longwave radiation, ground (W/m2)
   irveg                           , &  ! Absorbed longwave radiation, vegetation (W/m2)
                                        !
                                        ! *** Output ***
   ...
   flux_calc_switch = 0
!   if (flux_calculation == 1 .and. flag_tree_presence == 1 .and. year >= 7) then
...
if (flux_calculation == 1) then
write(*,*) 'sal (psu), din', year, doy, sal * 58.44d0 / 1000.0d0, din
end if
