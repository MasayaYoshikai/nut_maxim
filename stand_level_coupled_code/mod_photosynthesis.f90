...
contains
  ...
  subroutine photosynthesis_param ( &
    ...
    koha    = 36380.0d0
    cpha    = 37830.0d0
!    vcmaxha = 65330.0d0                               ! <= Bonan et al. (2014)
!    jmaxha  = 43540.0d0                               ! <= Bonan et al. (2014)
!    vcmaxha = (42.6d0 + 1.14d0 * t_growth) * 1000.0d0 ! <= Kumarathunge et al. (2019)
!    jmaxha  = 40710.0d0                               ! <= Kumarathunge et al. (2019)
...
!    vcmaxhd = 150000.0d0  ! <= Bonan et al. (2014)
!    jmaxhd  = 150000.0d0  ! <= Bonan et al. (2014)
...
!    vcmaxse = 490.0d0                                                   ! <= Bonan et al. (2014)
!    jmaxse  = 490.0d0                                                   ! <= Bonan et al. (2014)
!    vcmaxse = 645.13d0 - 0.38d0 * t_growth                              ! <= Kumarathunge et al. (2019)
!    jmaxse  = 658.77d0 - 0.84d0 * t_home - 0.52d0 * (t_growth - t_home) ! <= Kumarathunge et al. (2019)
  ...
  subroutine leaf_photosynthesis ( &
    ...
    do i = 1, n_layer
       if (leaf_layer(i) == 1) then ! Leaf layer
          ...
          an_err = (co2air - ci(i)) / (1.0d0 / gbc(i) + 1.6d0 / gs)
          ...
          hs(i) = (gbv(i)*eair + gs*esat) / ((gbv(i)+gs)*esat)
  ...
  function ci_func_gs (i, o2air, co2air, layer, flux) result(ci_val)
    ...
    if (leaf_layer(i) > 0.0d0) then ! leaf layer
       ...
       gleaf = 1.0d0 / (1.0d0/gbc(i) + 1.6d0/gs)
